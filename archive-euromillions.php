<?php
/**
 * Archive EuroMillions Template (Enhanced)
 * 
 * Template for displaying EuroMillions archive/listing page with modern styling
 */
...
.filter-button:hover {
    background: linear-gradient(135deg, #2a5298, #1a2f5a);
...
.view-details-btn, .check-numbers-btn {
    flex: 1;
    padding: 12px 20px;
    border-radius: 25px;
    text-decoration: none;
...
<div class="lottery-archive-container">
    ...
    <div class="lottery-archive-header">
        <h1 class="lottery-archive-title">
            ...
            EuroMillions Results Archive
        </h1>
        <p class="lottery-archive-subtitle">Browse through all EuroMillions draw results. Find winning numbers, prize breakdowns, and draw dates for every EuroMillions draw.</p>
    </div>
    
    <!-- Filter Section -->
    <div class="archive-filter-section">
        <h2 class="filter-title">🔍 Search Draws</h2>
        <div class="filter-grid">
            ...
            <div class="filter-item">
                ...
                <button class="filter-button" onclick="filterResults()">🔍 Search</button>
            </div>
        </div>
    ...
    <div class="lottery-archive-grid">
            ...
            <?php while (have_posts()) : the_post(); ?>
                ...
                <div class="lottery-archive-card">
                    <div class="card-header">
                        ...
                        if ($draw_date) : ?>
                            <div class="card-subtitle"><?php echo date('l - F j, Y', strtotime($draw_date)); ?></div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="winning-numbers-display">
                            ...
                            <div class="lottery-numbers">
                                ...
                                // Star numbers
                                $stars = get_post_meta(get_the_ID(), '_star_numbers', true);
                                if (is_array($stars) && !empty($stars)) {
                                    foreach (array_slice($stars, 0, 2) as $star) {
                                        echo '<div class="number-ball star-ball">' . esc_html($star) . '</div>';
                                    }
                                } elseif (!empty($stars)) {
                                    $star_array = explode(',', $stars);
                                    foreach (array_slice($star_array, 0, 2) as $star) {
                                        $clean_star = trim($star);
                                        if (!empty($clean_star)) {
                                            echo '<div class="number-ball star-ball">' . esc_html($clean_star) . '</div>';
                                        }
                                    }
                                }
                                ?>
                        ...
                        <div class="card-info">
                            <?php 
                            $prize_amount = get_post_meta(get_the_ID(), '_prize_amount', true)
                                         ?: get_post_meta(get_the_ID(), '_advertised_jackpot', true)
                                         ?: get_post_meta(get_the_ID(), '_jackpot', true);
                            if ($prize_amount) : ?>
                            <div class="info-item">
                                <div class="info-label">Jackpot</div>
                                <div class="info-value jackpot-value">€<?php echo number_format($prize_amount, 0); ?></div>
                            </div>
                            <?php endif; ?>
                        ...
                        <div class="card-actions">
                            <a href="<?php the_permalink(); ?>" class="view-details-btn">👁️ View Details</a>
                            <button class="check-numbers-btn" data-post-id="<?php echo get_the_ID(); ?>">🔍 Check Numbers</button>
                        </div>
                    </div>
                </div>
        ...
        <?php else : ?>
            ...
            <div class="no-results">
                ...
                <p>No EuroMillions draws found matching your criteria. Please try adjusting your filters.</p>
            </div>
    ...
    <div class="lottery-stats-section">
        <h2 class="stats-title">EuroMillions Statistics</h2>
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-number">€240M</div>
                <div class="stat-label">Largest Jackpot</div>
                <div class="stat-description">Record jackpot won in 2023</div>
            </div>
            
            <div class="stat-card">
                <div class="stat-number">2x</div>
                <div class="stat-label">Draws per Week</div>
                <div class="stat-description">Tuesday & Friday</div>
            </div>
            
            <div class="stat-card">
                <div class="stat-number">13</div>
                <div class="stat-label">Prize Categories</div>
                <div class="stat-description">Multiple ways to win</div>
            </div>
            
            <div class="stat-card">
                <div class="stat-number">1:139M</div>
                <div class="stat-label">Jackpot Odds</div>
                <div class="stat-description">Odds of winning jackpot</div>
            </div>
        </div>
    </div>
    ...
    <div class="lottery-info-section">
        <div class="lottery-info-grid">
            <div class="lottery-info-card">
                <h3>🎮 How to Play</h3>
                <p>EuroMillions is Europe's biggest lottery. Choose 5 main numbers from 1-50 and 2 Lucky Stars from 1-12 for a chance to win life-changing jackpots.</p>
                <a href="#" class="lottery-btn">Learn More</a>
            </div>
            
            <div class="lottery-info-card">
                <h3>🏆 Prize Categories</h3>
                <p>EuroMillions offers 13 prize categories. Match all 5 main numbers plus 2 Lucky Stars to win the jackpot, which can reach €240 million!</p>
                <a href="#" class="lottery-btn">View Prizes</a>
            </div>
            
            <div class="lottery-info-card">
                <h3>📅 Draw Schedule</h3>
                <p>EuroMillions draws take place every Tuesday and Friday at 21:00 CET. Don't miss your chance to become a millionaire!</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.check-numbers-btn').forEach(button => {
        button.addEventListener('click', function() {
            const postId = this.dataset.postId;
            // This would open a modal or redirect to a number checking page
            alert('Number checking functionality would be implemented here for post ID: ' + postId);
        });
    });
    
    // Set current date as default for "TO" field
